...
describe('metalsmith-markdown', function () {
  ...
  it('should be able to set the rendering environment per-page', function (done) {
    Metalsmith('test/fixtures/env-plugin')
      ...
      .use(
        markdown('default')
          .env(function (page, metadata) {
            return { title: page.title, siteName: metadata.siteName };
          })
          .use(function (md) {
            md.inline.ruler.push('vars', function (state, silent) {
              ...
              while (pos < max) {
                pos++;
                if (state.md.utils.isSpace(state.src.charCodeAt(pos))) {
                  break;
                }
              }
              if (!silent) {
            ...
            md.renderer.rules['vars'] = function (
              tokens,
              id,
              _unused_options,
              _unused_env,
      ...
      .build(function (err) {
        if (err) return done(err);
        equal(
          'test/fixtures/env-plugin/expected',
          'test/fixtures/env-plugin/build'
        );
        done();
      });
  });
  ...
  it('should accept plugin options', function (done) {
    Metalsmith('test/fixtures/plugin-options')
      ...
      .build(function (err) {
        if (err) return done(err);
        equal(
          'test/fixtures/plugin-options/expected',
          'test/fixtures/plugin-options/build'
        );
        done();
      });
  });
  ...
  it('should accept plugin options with a preset', function (done) {
    Metalsmith('test/fixtures/plugin-options-preset')
      ...
      .build(function (err) {
        if (err) return done(err);
        equal(
          'test/fixtures/plugin-options-preset/expected',
          'test/fixtures/plugin-options/build'
        );
        done();
      });
